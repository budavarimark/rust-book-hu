...
A Rust két nagy kategóriába sorolja a hibákat: helyrehozható és
helyrehozhatatlan hibákra. Egy _helyrehozható hiba_ (recoverable error) esetén –
ilyen például a _fájl nem található_ hiba – jó eséllyel csak jelenteni akarjuk a
problémát a felhasználónak, és újra megpróbálni a műveletet. A
_helyrehozhatatlan hibák_ (unrecoverable errors) mindig hibás kódra utalnak,
mint például egy tömb végén túli helyre való hivatkozás, ezért ilyenkor azonnal
le akarjuk állítani a programot.

A legtöbb nyelv nem tesz különbséget a hibák e két fajtája között, és mindkettőt
ugyanúgy kezeli, például kivételekkel. A Rustban nincsenek kivételek. Helyettük
a `Result<T, E>` típus áll rendelkezésre a helyrehozható hibákhoz, valamint a
